...
static LLVMRustError toRust(LLVMError Error) {
  switch (Error) {
  case NoDerivative:
    return LLVMRustErrorType::NoDerivative;
...
extern "C" LLVMValueRef LLVMRustEnzymeErrorHandler( const char *,
                                          LLVMValueRef V, ErrorType,
                                          const void *, LLVMValueRef,
                                          LLVMBuilderRef B) {
  // switch on error type:
  // We get a builder, insert custom code at point where the error occurs.
  //
  // NoDerivative => void ptr becomes gutils
  //
  /*
