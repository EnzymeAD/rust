use crate::ty::{self, Ty, Type, Kind, TypeTree, FncTree, FieldsShape};
...
fn typetree_from_ty<'a>(ty: Ty<'a>, tcx: TyCtxt<'a>, depth: usize, safety: bool, visited: &mut Vec<Ty<'a>>, span: Option<Span>) -> TypeTree {
    // TODO: Improve Enzyme Core TypeTrees to be able to have a larger depth without blowing up
    // compile times.
    if depth > 7 {
        trace!("depth > 7 for ty: {}", &ty);
    }
